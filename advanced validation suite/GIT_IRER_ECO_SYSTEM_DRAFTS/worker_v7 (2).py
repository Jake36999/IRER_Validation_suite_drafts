...
def run_simulation(N_grid: int, L_domain: float, T_steps: int, DT: float, ALPHA: float, KAPPA: float, SIGMA_K: float, D: float, ETA: float) -> Tuple[SimState, jnp.ndarray]:
    """
    ...
    timesteps = jnp.arange(T_steps) * DT
...
def main():
    ...
    try:
        final_state, rho_history = run_simulation(
            N_grid=N_GRID,
            L_domain=L_DOMAIN,
            T_steps=T_STEPS,
            DT=DT,
